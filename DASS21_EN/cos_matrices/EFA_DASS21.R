...
models = c('distilroberta', 'mpnet', 'miniLM', 'e5', 'Wulff', 'mpnet_en', 't5')
...
for (i in 1:length(models)){
    item_embed[[i]] = symm(as.matrix(read.csv(paste0('./DASS21_EN/cos_matrices/matrix_concatenated_item_', models[i], '.csv'))),var_order)
}
...
models = c('distilroberta', 'mpnet', 'miniLM', 'e5','Wulff', 'mpnet_en', 't5','avg_transf')
...
write.csv(round(targ_load_item_labeled,3), file = './DASS21_EN/Results_target_item_labeled.csv')
write.csv(round(targ_cor_item_labeled,3), file = './DASS21_EN/Results_target_item_labeled_cor.csv')
