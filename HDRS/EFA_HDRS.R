...
var_order = c("Umore.depresso", "lavoro.e.attività", "rallentamento..ideomotorio..linguaggio..difficoltà.a.concentrarsi.",  "presa.di.coscienza", #Core depression
              "Sentimenti.di.colpa", "agitazione", "ansia.psichica", #Ansia psicologica
              "Insonnia.iniziale", "Insonnia.centrale", "Insonnia.ritardata",  "ansia.somatica..concomitanti.fisici.dell.ansia.", "ipocondria", #Ansia somatica
              "sintomi.somatici.gastrointestinali", "perdita.di.peso") #Anoressia

#create a list which will be necessary for labeling factors later on
label = list(c(var_order[1:4]), c(var_order[5:7]), c(var_order[8:11]), c(var_order[12:13]))
names(label) <- c("core", "psic", "som", "anor")
...
for (i in 1:length(models)){
    item_embed[[i]] = symm(as.matrix(read.csv(paste0('./cos_matrices/matrix_concatenated_item_', models[i], '.csv'))),var_order)
}
...
#specify initial target matrix according to empirical validation study results
                      #H#E#X#A#C#O
target.mat = as.matrix(cbind(c(rep(1,4),rep(0,10)),
                             c(rep(0,4),rep(1,3), rep(0,7)),
                             c(rep(0,7),rep(1,5), rep(0,2)),
                             c(rep(0,12),rep(1,2))))
...
emp_esem_target_efa = matrix(NA, nrow = 14, ncol =4)
colnames(emp_esem_target_efa) = names(label)
rownames(emp_esem_target_efa) = var_order
emp_esem_target_Phi = matrix(rnorm(16), ncol = 4, nrow = 4)
...
for (i in 1:(length(models)+1)){
   x = i*4 #multiple i*6 to select 6 columns at a time
   if (i == 1){
  #loadings
  print(NA)
  #cor
  ...
  #loadings
   targ_load_item_labeled <- cbind(targ_load_item_labeled, round(label_factors_enhanced(targ_load_item[,(x-3):x], group = label, mod = paste0('_',models[i-1]), cor_matrix = targ_cor_item[,(x-4):x])$labeled_loading_matrix,3))
  #cor
    targ_cor_item_labeled <- cbind(targ_cor_item_labeled, round(label_factors_enhanced(targ_load_item[,(x-3):x], group = label, mod = paste0('_',models[i-1]), cor_matrix = targ_cor_item[,(x-4):x])$labeled_cor_matrix,3))
   }
}
