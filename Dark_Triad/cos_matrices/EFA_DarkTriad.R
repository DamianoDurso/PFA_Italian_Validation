...
for (i in 1:length(models)){
    item_embed[[i]] = symm(as.matrix(read.csv(paste0('./Dark_Triad/cos_matrices/matrix_concatenated_item_', models[i], '.csv'))),var_order)
}
...
write.csv(round(targ_load_item_labeled,3), file = './Dark_Triad/Results_target_item_labeled.csv')
write.csv(round(targ_cor_item_labeled,3), file = './Dark_Triad/Results_target_item_labeled_cor.csv')
